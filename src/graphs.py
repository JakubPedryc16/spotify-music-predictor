...
def plot_pairplot_music(df: pd.DataFrame, out_path: Path, max_rows: int = 2000) -> None:
    selected = ["popularity", "tempo", "energy", "danceability",
                "loudness", "speechiness", "acousticness",
                "instrumentalness", "liveness", "valence", "duration_min"]
    selected = [c for c in selected if c in df.columns]
    if not selected:
        return
    df_num = df[selected].dropna()
    if df_num.empty:
        return
...
def plot_boxplots(df: pd.DataFrame, out_path: Path) -> None:
    df = df.copy()
    selected = ["popularity", "tempo", "energy", "danceability", "loudness", "speechiness", "acousticness",
                "instrumentalness", "liveness", "valence", "duration_min"]
    selected = [c for c in selected if c in df.columns]
    if not selected:
        return
...
def run_analysis(show: bool = False) -> None:
    ...
    variants = ["_minmax", "_standardized"]  # tylko te warianty
    for base in base_names:
        for var in variants:
            ...
            plot_pairplot_music(df, save_dir / "pairplot_music.png")
    ...
    print("Struktura: wyniki/<nazwa_zbioru>/{pairplot_music.png, corr_heatmap_*.png, boxplots.png}")
